...
-- Remove Standard.error.of.ladder.score, upperwhiser, and lowerwhisker columns -- Rename columns for more intuitive understanding
...
-- Load global top spotify song dataset -- Select name, artists, country, danceability ,energy, loudness, valence, and tempo --Rename name column as title
...
CREATE TABLE ireland_10_16_25 AS
  SELECT *
  FROM read_csv('10_16_25_ireland.csv')
```

```{sql}
#| connection: con
SELECT *
FROM ireland_10_16_25
```
...
CREATE TABLE ireland_25_clean AS
SELECT
    TRIM(artist) AS artist,
    TRIM(title) AS title
FROM ireland_25;
...
CREATE TABLE spotify_2025_unique AS
SELECT DISTINCT title, artist, danceability, energy, loudness, valence, tempo
FROM spotify_2025_sql;
```
...
#| connection: con
SELECT *
FROM spotify_2025_unique
```

```{sql}
#| connection: con
SELECT DISTINCT '"' || artist || '"' FROM new_zealand_25_clean;
```


-- Combine dataset 2 and 3 - replace country abbreviation with its full name \## Match spotify's country abbreviation with abbreviation table's abbreviation \## Remove any unmatched rows
...
CREATE TABLE new_zealand_25_matched_sql AS
  ...
  LEFT JOIN spotify_2025_unique AS s
  ON nz.title = s.title
...
CREATE TABLE ireland_25_matched_sql AS
  SELECT i.*, s.*
  FROM ireland_25_clean AS i
  LEFT JOIN spotify_2025_unique AS s
  ON i.title = s.title
    AND i.artist = s.artist;
    
ALTER TABLE ireland_25_matched_sql
DROP COLUMN artist_1;
...
CREATE TABLE ireland_spotify_2025_avg AS 
  SELECT AVG(danceability) AS danceability_avg,
         AVG(energy) AS energy_avg,
         AVG(loudness) AS loudness_avg,
         AVG(valence) AS valence_avg,
